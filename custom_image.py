...
This code is based of fchollet's ImageDataGenerator for Keras,
and forked from oeway's modifications to include tiff file input
and reading from images with an arbitrary number of channels,
as well as introduced compatibility for random/center cropping,
tbls, npy reading, and advanced iteration for proper class representation
See: https://github.com/fchollet/keras/issues/3338
'''
...
def random_zoom(x, zoom_range, row_index=0, col_index=1, channel_index=2,
                fill_mode='nearest', cval=0.):
    if len(zoom_range) != 2:
...
def random_channel_shift(x, intensity, channel_index=2):
...
def tiff_img_to_array(img, dim_ordering=K.image_dim_ordering()):
    """Changes a tiff image to proper array orientation
    Separate from img_to_array because tiff imreader outputs different order
    """
    if dim_ordering not in ['th', 'tf']:
        raise Exception('Unknown dim_ordering: ', dim_ordering)
    # tiff image has dim_ordering (channel, height, width)
...
def load_npy(path, target_mode=None, target_size=None):
    img = np.load(path)
    # TODO: Support resizing
    return img

def load_tiff_img(path, target_mode=None, target_size=None):
...
def load_tbl(path, target_mode=None, target_size=None):
    with tables.open_file(path, 'r') as h5_file:
        img = h5_file.root.carray.read()

    return img

def list_pictures(directory, ext='jpg|jpeg|bmp|png|tiff|tif|npy|tbl'):
...
def tiff_image_reader(filepath, target_mode=None, target_size=None, dim_ordering=K.image_dim_ordering(), **kwargs):
    img = load_tiff_img(filepath, target_mode=target_mode, target_size=target_size)
    return tiff_img_to_array(img, dim_ordering=dim_ordering)

def npy_image_reader(filepath, target_mode=None, target_size=None, dim_ordering=K.image_dim_ordering(), **kwargs):
    img = load_npy(filepath, target_mode=target_mode, target_size=target_size)
    return img_to_array(img, dim_ordering=dim_ordering)

def tbl_image_reader(filepath, target_mode=None, target_size=None, dim_ordering=K.image_dim_ordering(), **kwargs):
    img = load_tbl(filepath, target_mode=target_mode, target_size=target_size)
    return img_to_array(img, dim_ordering=dim_ordering)
...
def pick_channels(x, channel_idxs, **kwargs):
    """Returns the channels listed in channel_idxs
    note that if channel_idxs is None, it returns all of the channels,
    which is the behavior we want

    If -1 is in the list channel_idxs, then we remove it from the list and append
    a mask of the unlabeled pixels for this particular selection of channels
    """
    if channel_idxs is not None:
        if -1 in channel_idxs:
            channel_idxs_tmp = list(channel_idxs)
            channel_idxs_tmp.remove(-1)
            # return mask of all pixels that are not labeled for this
            # channel selection
            blank_pixel_mask = 1 - np.max(x[:, :, tuple(channel_idxs_tmp)],
                                          axis=2,
                                          keepdims=True)
            # concatenate to other masks for softmax calculation
            x = np.concatenate((blank_pixel_mask,
                               x[:, :, tuple(channel_idxs_tmp)]),
                               axis=2)
        else:
            x = x[:, :, tuple(channel_idxs)]

    return x

def get_max_class(x, **kwargs):
    """Returns a one-hot encoding of the class most represented in 
    the image"""
    #print(x.shape)
    channel_dim = x.shape[2]+1
    class_sums = np.sum(x, axis=(0,1))
    one_hot = np.zeros((channel_dim))
    # first channel reserved for if no instances are represented
    if np.max(class_sums) == 0:
        one_hot[0] = 1
    else:
        max_class = np.argmax(np.sum(x, axis=(0,1)))
        one_hot[max_class+1] = 1
    return one_hot

def get_soft_class(x, **kwargs):
    """Returns soft targets for classes in image (proportional area
    of each class, that is, target[class] = area[class]/(area[all classes])
    where a completely unlabeled pixel is a class as well.
    """
    channel_dim = x.shape[2]+1
    # total area of completely unlabeled pixels
    unlabeled = np.sum(1-np.max(x, axis=2))
    pixel_sum = unlabeled + np.sum(x)
    soft_class = np.zeros((channel_dim))
    soft_class[0] = unlabeled / pixel_sum
    soft_class[1:] = np.sum(x, axis=(0,1)) / pixel_sum

    return soft_class

def reshape_for_keras(x, **kwargs):
    """Returns a flattened array so that keras can take the appropriate softmax
    See https://github.com/fchollet/keras/issues/1169
    """
    (W, H, C) = x.shape
    return np.reshape((W*H, C))
...
class ImageDataGenerator(object):
    ...
    def __init__(self,
        ...
        self.__sync_seed = self.config['seed'] or np.random.randint(0, 2147483646)
    ...
    def extend_pipeline(self, p):
        if p is not None:
            self.__pipeline.extend(p)
    ...
    def flow_from_directory(self, directory,
                            ...
                            read_formats={'png','jpg','jpeg','bmp','tiff','tif', 'npy', 'tbl'},
    ...
    def process(self, x):
        ...
        self.__sync_seed = np.random.randint(0, 2147483646)
...
class Iterator(object):
    ...
    def _flow_index(self, N, batch_size=32, shuffle=False, seed=None):
        ...
        self.reset()
        while 1:
            # There was a bug in the following section which cauaed batches
            # to be incomplete. This may have been intended to be a "feature"
            # but it didn't function normally because the total sample
            # calculation was incorrect. Instead we just let the indices wrap
            # around to the beginning, re-sampling the first images in the data
            # generator. For most applications this is fine, as the image list
            # to draw from will be reshuffled in the next step.
            ...
            if N >= current_index + batch_size:
                self.batch_index += 1
                index_array = self.index_array[current_index : current_index + current_batch_size]

            else:
                res_batch_size = N - current_index
                self.batch_index = 0 
                index_array = np.append(self.index_array[current_index:], 
                                        self.index_array[:self.batch_size - res_batch_size])
    ...
    def __add__(self, it):
        ...
        seed = self.seed or np.random.randint(0, 2147483646)
    ...
    def next(self):
        ...
        if self.save_to_dir:
            for i in range(current_batch_size):
                ...
                if self.save_mode == 'tiff':
                    tiff.imsave(os.path.join(self.save_to_dir, fname), img)
                elif self.save_mode == 'npy':
                    np.save(os.path.join(self.save_to_dir, fname), img)
                elif self.save_mode == 'tbl':
                    FILTERS = tables.Filters(complib='blosc', complevel=5)
                    with tables.open_file(os.path.join(self.save_to_dir, fname), mode='w', filters=FILTERS) as h5_file:
                        h5_file.create_carray('/', 'carray', obj=img)
                else:
...
class DirectoryIterator(Iterator):
    ...
    def __init__(self, directory, image_data_generator,
                 color_mode=None, target_size=None,
                 image_reader='pil', read_formats={'png','jpg','jpeg','bmp', 'tif', 'tiff', 'npy', 'tbl'},
        ...
        elif self.image_reader == 'tiff':
            self.image_reader = tiff_image_reader
        elif self.image_reader == 'npy':
            self.image_reader = npy_image_reader
        elif self.image_reader == 'tbl':
            self.image_reader = tbl_image_reader
